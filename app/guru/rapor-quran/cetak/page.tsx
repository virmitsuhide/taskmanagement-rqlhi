...
export default async function CetakRaporPage({ searchParams }: PageProps) {
  ...
  const bahan = (await getBahanRaporSesi(sesi, term as Semester, templates)).filter(b => b.template)
  const belum = bahan.filter(b => !b.deskripsi.trim()).length
    ...
    <div className="min-h-screen" style={{ background: 'var(--secondary)' }}>
      <div className="mx-auto max-w-4xl space-y-4 px-4 py-6 md:px-6 print:max-w-none print:p-0">
        <div className="print:hidden">
          <Link
            href={`/guru/rapor-quran?sesi=${sesi.id}&term=${term.id}`}
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Semester {term.semester === 'ganjil' ? 'Ganjil' : 'Genap'} {term.year_label}. Tiap anak satu halaman.
            {belum > 0 && ` ${belum} di antaranya belum berdeskripsi.`}
          </p>
          <div className="mt-3">
