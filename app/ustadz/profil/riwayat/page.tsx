            ...
            {riwayat.map(r => (
              <tr key={`${r.year}-${r.month}`} className="border-t hover:bg-muted/30">
                ...
                <td className="px-2 py-2 text-center">
                  <span className={cn('inline-block whitespace-nowrap rounded px-1.5 py-0.5 font-medium', LEVEL_TONE[r.level])}>
                    {r.level} · {r.predikat}
                  </span>
                </td>
              </tr>
      ...
      {berisi.map(r => (
        <div key={`${r.year}-${r.month}`} className="overflow-hidden rounded-xl border bg-card shadow-sm">
          <div className="flex items-center gap-2 border-b bg-muted/40 px-4 py-2.5">
            ...
            <span className={cn('rounded px-1.5 py-0.5 text-[11px] font-medium', LEVEL_TONE[r.level])}>
